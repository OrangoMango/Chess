...
public class Piece{
	...
	public static enum Pieces{
		PAWN(PIECE_PAWN, 1, MOVE_HORIZONTAL),
		ROOK(PIECE_ROOK, 8, MOVE_HORIZONTAL),
		KNIGHT(PIECE_KNIGHT, 1, MOVE_KNIGHT),
		BISHOP(PIECE_BISHOP, 8, MOVE_DIAGONAL),
		KING(PIECE_KING, 1, MOVE_HORIZONTAL, MOVE_DIAGONAL),
		QUEEN(PIECE_QUEEN, 8, MOVE_HORIZONTAL, MOVE_DIAGONAL);
		
		private String name;
		private int[] directions;
		private int amount;
		
		private Pieces(String name, int amount, int... directions){
			this.name = name;
			this.amount = amount;
	...
	public Piece(Pieces type, int color, int x, int y){
		this.type = type;
		this.color = color;
		this.x = x;
		this.y = y;
	...
	public String toString(){
		return String.format("%s(%d)", this.type.getName(), this.color);
	}
}
