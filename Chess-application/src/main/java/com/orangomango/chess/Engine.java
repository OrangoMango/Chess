...
public class Engine{
	...
	public String getBestMove(Board b){
		...
		writeCommand(MainApplication.format("go wtime %s btime %s winc %s binc %s", b.getTime(Color.WHITE), b.getTime(Color.BLACK), b.getIncrementTime(), b.getIncrementTime()));
		String output = "";
		while (true) {
			try {
				String line = this.reader.readLine();
